...
const Settings = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.emailConfig.user && formData.emailConfig.password) {
        const response = await fetch(`${process.env.REACT_APP_API_URL || ''}/api/update-email-config`, {
          method: 'POST',
          headers: {
  ...
  const sendTestEmail = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || ''}/api/send-single-reminder`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
